...
- 按类型筛选：画廊顶部有分类行（内容搜索、热榜榜单、账号分析、评论分析、信息源订阅、视频下载、文案创作、违禁检测、AI 生成、AI 搜索、财经投资、效率工具），卡片上也会显示分类标签。
...
3. 点顶部的分类标签（全部 / 内容搜索 / 热榜榜单 / …）按类型筛选，可与搜索叠加使用。
4. 在顶部搜索框输入关键词，按中文名 / slug / 描述过滤。
5. 点击某张卡片，右侧滑出详情面板，查看该技能的 README（可切换中文 / 英文）。
6. 点击卡片上的「立即使用」，技能会以 `/name` 的形式引用到输入框，补充或修改你的需求后回车发送。
7. 按 ESC 或右上角「关闭」返回会话。

> 若尚未安装 `redfox-community-dsh`，打开画廊时会自动执行安装；安装完成后仍需重启 `dsh web` 才能生效，因此更推荐提前安装。

## 技能分类维护

分类数据存放在本插件的 `lib/skill-categories.json`：

- `categories`：分类定义（`id` / 中文名 / 英文名），数组顺序即界面筛选行的展示顺序。
- `skills`：skill 名称到分类 id 的预分类映射，首次安装后即可直接按类型划分。
- `fallbackRules`：关键词兜底规则，未进入映射的新 skill 会先按规则自动归类，仍不命中则归入「其他」。

`redfox-community-dsh` 后续新增 skill 时，由 agent 判断其类型，在 `skills` 中追加一行 `"<skill-name>": "<category-id>"` 即可；接口每次请求都会重新读取该文件，无需重启 `dsh web` 即可生效。
