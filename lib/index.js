 ...
 *  - GET  `/redfox-skill-gallery/redfox-skills` -> `{ installed, count, skills, categories }`
 ...
 *
 * Each skill carries a `category` id resolved from `skill-categories.json`
 * (explicit `skills` map first, then keyword `fallbackRules`, else `other`).
 * The JSON is re-read on every list request so agent-side re-classification
 * of newly added skills takes effect without restarting `dsh web`.
 */
...
const CATEGORY_FILE = join(dirname(fileURLToPath(import.meta.url)), 'skill-categories.json')
...
/**
 * Load the category registry (`skill-categories.json`). Called per request so
 * agent edits that classify newly added skills apply without a host restart.
 */
function loadCategoryData() {
  try {
    const data = JSON.parse(readFileSync(CATEGORY_FILE, 'utf8'))
    return {
      categories: Array.isArray(data.categories) ? data.categories : [],
      skills: data.skills && typeof data.skills === 'object' ? data.skills : {},
      fallbackRules: Array.isArray(data.fallbackRules) ? data.fallbackRules : [],
    }
  } catch {
    return { categories: [], skills: {}, fallbackRules: [] }
  }
}

/** Resolve a skill's category id: explicit map -> keyword rules -> `other`. */
function classifySkill(registry, name, description) {
  const mapped = registry.skills[name]
  if (typeof mapped === 'string' && mapped) return mapped
  const haystack = `${name} ${description || ''}`.toLowerCase()
  for (const rule of registry.fallbackRules) {
    if (!rule || !rule.category || !rule.pattern) continue
    try {
      ...
      const offList = hostCtx.webServer.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            const registry = loadCategoryData()
            const skills = redfox.map((s) => {
              ...
              return {
                ...
                zhName,
                category: classifySkill(registry, s.name, s.description),
              }
            })
            sendJson(res, 200, { installed: redfox.length > 0, count: skills.length, skills, categories: registry.categories })
          } catch (err) {
